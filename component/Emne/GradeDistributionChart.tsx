...
export function GradeDistributionChart({ grades, selectedSemester, onSemesterChange }: GradeDistributionChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    if (hasLetterGrades) {
      return [
        { grade: 'A', count: selectedGradeData.gradeA || 0, fill: 'var(--chart-1)' },
        { grade: 'B', count: selectedGradeData.gradeB || 0, fill: 'var(--chart-2)' },
        { grade: 'C', count: selectedGradeData.gradeC || 0, fill: 'var(--chart-3)' },
        { grade: 'D', count: selectedGradeData.gradeD || 0, fill: 'var(--chart-4)' },
        { grade: 'E', count: selectedGradeData.gradeE || 0, fill: 'var(--chart-5)' },
        { grade: 'F', count: selectedGradeData.gradeF || 0, fill: 'var(--chart-6)' },
      ]
    } else {
      return [
        { grade: 'Pass', count: selectedGradeData.gradePass || 0, fill: 'var(--chart-1)' },
        { grade: 'Fail', count: selectedGradeData.gradeFail || 0, fill: 'var(--chart-6)' },
      ]
    }
  }, [selectedGradeData])
  ...
  const passRate = useMemo(() => {
    ...
    if (hasLetterGrades) {
      const passing = (selectedGradeData.gradeA || 0) + 
        ...
        <div className="h-[300px]">
          {selectedGradeData ? (
            ...
            <div className="flex h-full items-center justify-center">
              <p className="text-muted-foreground">Select a semester to view grade distribution</p>
            </div>
          )}
        </div>
      </CardContent>
