...
export function GradeStatisticsCards({ grades }: GradeStatisticsCardsProps) {
  ...
  const previousGrade = useMemo(() => {
    return sortedGrades.length > 1 ? sortedGrades[sortedGrades.length - 2] : null
  }, [sortedGrades])
  ...
  const passRate = useMemo(() => {
    ...
    if (hasLetterGrades) {
      const passing = (latestGrade.gradeA || 0) + 
                     (latestGrade.gradeB || 0) + 
                     (latestGrade.gradeC || 0) + 
                     (latestGrade.gradeD || 0) + 
                     (latestGrade.gradeE || 0)
      currentPassRate = Math.round((passing / latestGrade.participantsTotal) * 100)
      
      if (previousGrade) {
        const prevPassing = (previousGrade.gradeA || 0) + 
                           (previousGrade.gradeB || 0) + 
                           (previousGrade.gradeC || 0) + 
                           (previousGrade.gradeD || 0) + 
                           (previousGrade.gradeE || 0)
        previousPassRate = Math.round((prevPassing / previousGrade.participantsTotal) * 100)
      }
    } else {
      currentPassRate = Math.round(((latestGrade.gradePass || 0) / latestGrade.participantsTotal) * 100)
      
      if (previousGrade) {
        previousPassRate = Math.round(((previousGrade.gradePass || 0) / previousGrade.participantsTotal) * 100)
      }
    }
  ...
  const participantsTrend = useMemo(() => {
    ...
    return {
      count: currentCount,
      change: previousCount ? Math.round(((currentCount - previousCount) / previousCount) * 100) : 0
    }
  ...
  return (
    <>
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{latestGrade.participantsTotal}</div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center">
            ...
            <span className={passRate.change > 0 ? "text-emerald-500" : passRate.change < 0 ? "text-red-500" : "text-muted-foreground"}>
              {passRate.change > 0 ? '+' : ''}{passRate.change}%
            </span>
          </div>
          <p className="text-xs text-muted-foreground">From previous semester</p>
        </CardContent>
      </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center">
                ...
                <span className={participantsTrend.change > 0 ? "text-emerald-500" : participantsTrend.change < 0 ? "text-red-500" : "text-muted-foreground"}>
                  {participantsTrend.change > 0 ? '+' : ''}{participantsTrend.change}%
                </span>
              </div>
              <p className="text-xs text-muted-foreground">From previous semester</p>
            </CardContent>
